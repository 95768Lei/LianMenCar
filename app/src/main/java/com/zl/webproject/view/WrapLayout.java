    ...
    private void createChild(String[] data, final Context context, int textSize, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (style == TEXTVIEW_STYLE) {
                ...
                ((TextView) btn).setTextColor(getResources().getColor(R.color.hint_color));
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < lineNums; i++) {
            ...
            for (int j = 0; j < lineViews.size(); j++) {
                ...
                ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams) lineChild.getLayoutParams();
